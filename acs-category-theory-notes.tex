...
\chapter{Morphisms}
\lecturedetails{19 October 2017}{M Fiore, D Szamozvancev, S Lau}

\section{Isomorphisms}
\begin{definition}[Isomorphism]
An arrow $f : A \morpharrow B$ is an \emph{isomorphism} if there exists an
inverse arrow $g : B \morpharrow A$ such that
\[
    g \comp f = \id{A} \quad \text{ and } \quad f \comp g = \id{B}
\]
We say $A$ and $B$ are \emph{isomorphic} if there exists an isomorphism between
them.
\end{definition}
...
Then
\begin{alignat*}{4}
    h \comp (f \comp g) &= h \comp \id{B} &&= h \quad \text{and} \\
...
\begin{minipage}[t]{0.50\textwidth}
    ...
    \begin{tikzpicture}
        ...
        \node (a) [dot] {};
        \node (b) [dot, below=of a] {};
        \node (p) [dot, right=50pt of a] {};
        \node (q) [dot, below=of p] {};
        \node (r) [dot, right=10pt of p] {};
        \node (s) [dot, below=of r] {};
        \node (t) [dot] at ($(r)!1/2!(s)$) {};
        \foreach \i/\j in {a/p,b/q}
